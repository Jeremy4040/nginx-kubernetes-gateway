...
func NewChangeProcessorImpl(cfg ChangeProcessorConfig) *ChangeProcessorImpl {
	return &ChangeProcessorImpl{
		store:                newStore(),
		relationshipCapturer: relationship.NewCapturer(),
		cfg:                  cfg,
	}
}
...
func (c *ChangeProcessorImpl) CaptureUpsertChange(obj client.Object) {
	...
	c.relationshipCapturer.Capture(obj)
	...
	c.store.changed = c.store.changed || c.relationshipCapturer.Exists(obj, client.ObjectKeyFromObject(obj))
}
...
func (c *ChangeProcessorImpl) CaptureDeleteChange(resourceType client.Object, nsname types.NamespacedName) {
	...
	c.store.changed = c.store.changed || c.relationshipCapturer.Exists(resourceType, nsname)

	c.relationshipCapturer.Remove(resourceType, nsname)
}
