...
// (1) We don't check generation here because Services do not use generation, and we are filtering upsert events based on the Service ports. We will only receive upsert events for Services with port changes.
// (2) We don't set the store's changed value to true because we don't want to trigger a reload on every Service changes. We will rely on the relationship.Capturer to trigger a reload when necessary.
